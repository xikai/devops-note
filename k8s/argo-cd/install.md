...
* [repositories配置声明](https://argo-cd.readthedocs.io/en/stable/operator-manual/declarative-setup/#repositories)
```yml
# application.yaml
apiVersion: v1
kind: Secret
metadata:
  name: repo-creds-argocd-example-app
  namespace: argocd
  labels:
    argocd.argoproj.io/secret-type: repository
stringData:
  type: git
  url: https://codeup.aliyun.com/newvevor/share-devops/argocd-example-app.git
  username: admin
  password: 123456
---
...
spec:
  source:
    repoURL: https://codeup.aliyun.com/newvevor/share-devops/argocd-example-app.git
    targetRevision: HEAD
    path: dev
    #directory:
    #  recurse: true  #递归目录
  ...
  project: default
  syncPolicy:     #指定自动同步策略和频率，不配置时需要手动触发同步; Argo CD 默认情况下每 3 分钟会检测 Git 仓库一次，用于判断应用实际状态是否和 Git 中声明的期望状态一致，如果不一致，状态就转换为 OutOfSync。默认情况下并不会触发更新，除非通过 syncPolicy 配置了自动同步
    syncOptions:  #定义同步方式
